...
function getForwardVector() {
  if (!app?.player) return playerDirection.set(0, 0, -1);
...
function getSideVector() {
  if (!app?.player) return playerDirection.set(1, 0, 0);
  app.player.getWorldDirection(playerDirection);
  playerDirection.y = 0;
...
function updatePlayer(deltaTime: number) {
  ...
  app.player.position.copy(
    new THREE.Vector3().subVectors(playerCollider.start, playerFixVector),
  );
}
...
function changeAction(actionName: string) {
  if (!app?.allActions) return;
  const next = app.allActions[actionName];
  if (!next || app.currentAction?.getClip().name === actionName) return;
